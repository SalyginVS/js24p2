...
const t04 = () => {
    let value = document.querySelector('.s-4').value;
    st_04.delete(value);
...
const t06 = () => {
    let arr = [...st_06][0]; // получаем массив из Set
    st_06.clear(); // очищаем Set
    arr.forEach(item => st_06.add(item)); // добавляем элементы массива в Set
    console.log(st_06); 
}
...
const t07 = () => {
    let arrays = [...st_07]; // преобразуем Set в массив массивов
    st_07.clear(); // очищаем Set
    arrays.forEach(arr => {
        arr.forEach(item => st_07.add(item)); // добавляем каждый элемент каждого массива в Set
    });
    //    st_07 = new Set([...st_07].flat()); // альтернативный способ с использованием flat для плоских массивов в одну строку
        console.log(st_07);
}
...
const t10 = () => {
    return new Set([...st_10].filter(item => item % 2 !== 0));
}
...
const t11 = () => {
    return a.union(b);
    // Альтернатива return new Set([...a, ...b]);
}
...
const t12 = () => {
    ...
    // Альтернатива: return new Set([...a].filter(item => !b.has(item)));
}
...
const t14 = () => {
    ...
    // Альтернатива: 
    // let diff1 = [...a].filter(item => !b.has(item));
    // let diff2 = [...b].filter(item => !a.has(item));
...
const t15 = () => {
    // Способ 1: filter и новый Set (короткий)
    st_15 = new Set([...st_15].filter(item => item <= 10));
    return st_15;
    
    // Способ 2: forEach с delete (текущий)
    // [...st_15].forEach(item => {
    //     if (item > 10) {
    //         st_15.delete(item);
    //     }
...
const t17 = () => {
    let setB = new Set(barr_17);
    return farr_17.filter(item => setB.has(item));
}
...
const t18 = () => {
    let setB = new Set(barr_18);
    return farr_18.filter(item => !setB.has(item));
}
...
const t19 = () => {
    let setA = new Set(farr_19);
    let setB = new Set(barr_19);
    let diff1 = farr_19.filter(item => !setB.has(item));
    let diff2 = barr_19.filter(item => !setA.has(item));
    return [...diff1, ...diff2];
}
